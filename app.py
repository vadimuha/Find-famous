...
def select_type(table,column_name,search_by,query):
	db.execute("SELECT DISTINCT {} FROM {} WHERE {} LIKE ? GROUP BY {}".format(column_name,table,search_by,column_name),[('%{}%'.format(query))])
	result = db.fetchall()
	return result
...
def get_actor():
	if request.method == "GET":
		...
		actors  = select_type("data","actor_1_name","actor_1_name",name)
		actors += select_type("data","actor_2_name","actor_2_name",name)
		actors += select_type("data","actor_3_name","actor_3_name",name)
